const DARK_RUN = {
    ...
    mass_glyph_eff(i) {
        let x,
         g = (tmp.c16active || CHALS.inChal(17) || FERMIONS.onActive("18")? i == 5 ? hasCharger(10)?5:FERMIONS.onActive("18")?100000:10 : hasCharger(10)?50:FERMIONS.onActive("18")?100000:10 : player.dark.run.glyphs[i]) / tmp.dark.glyph_weak
...
function updateDarkRunHTML() {
    ...
    for (let x = 0; x < MASS_GLYPHS_LEN; x++) {
        tmp.el["mass_glyph"+x].setHTML(
            tmp.c16active || CHALS.inChal(17) || FERMIONS.onActive("18")? x == 5 ? hasCharger(10)?5:FERMIONS.onActive("18")?100000:10 : hasCharger(10)?50:FERMIONS.onActive("18")?100000:10 : player.dark.run.glyphs[x]
...
function updateDarkRunTemp() {
    let dtmp = tmp.dark
    let dra = player.dark.run.active
    if (tmp.c16active || CHALS.inChal(17) || FERMIONS.onActive('18')) player.dark.run.active = player.dark.run.active
