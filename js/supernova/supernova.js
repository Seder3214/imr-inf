const SUPERNOVA = {
    ...
    req(x=player.supernova.times) {
        ...
        maxlimitGal = E('ee35000').pow(player.galaxy.times.scaleEvery('galaxy',false).pow(3e4).root(hasElement(310)?elemEffect(310):1)).mul('ee24500')
        bulk = E(0)
        bulkGal = E(0)
        if (player.stars.points.div('ee24500').gte(1)) bulkGal = player.stars.points.div('ee24500').max(1).log('ee35000').max(0).root(3e4).pow(hasElement(310)?elemEffect(310):1).scaleEvery('galaxy',true).add(1).floor()
